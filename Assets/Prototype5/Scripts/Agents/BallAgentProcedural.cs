...
public class BallAgentProcedural : Agent
{
    [Header("References")]
    ...
    public Transform levelGeneratorProcedural;
    Rigidbody rgbd;
    Collider col;
    ...
    public float targetReachedBonus;
    public float movingToTargetReward;
    public float notMovingToTargetPenalty;
    // Runtime variables
    [SerializeField]
    float currentBounce;
    ...
    public override void OnEpisodeBegin()
    {
        ...
        levelGeneratorProcedural.GetComponent<LevelGeneratorProcedural>().Generate();
        currentTargetIndex = 0;
        currentTarget = levelGeneratorProcedural.transform.GetChild(currentTargetIndex +1).gameObject;
    ...
    public override void OnActionReceived(ActionBuffers actions)
    {
        ...
        if(currentTarget != null){
        float currentTargetDistance = Vector3.Distance(new Vector3(transform.localPosition.x, 0f, transform.localPosition.z), new Vector3(currentTarget.transform.localPosition.y, 0f, currentTarget.transform.localPosition.z));
            if(currentTargetDistance < minTargetDistance){
                minTargetDistance = currentTargetDistance;
                AddReward(movingToTargetReward);
            }
            else{
                AddReward(notMovingToTargetPenalty);
            }
        }
        ...
        if(other.gameObject.layer == LayerMask.NameToLayer("Death")){
            ...
            if(other.gameObject.tag == "Platform"){
                if(other.gameObject == currentTarget){
                    AddReward(targetReachedBonus);
                    currentTarget.GetComponent<MeshRenderer>().material = rewardAttainedMaterial;
                    currentTargetIndex++;
                    currentTarget = levelGeneratorProcedural.transform.GetChild(currentTargetIndex +1).gameObject;
                    minTargetDistance = Vector3.Distance(new Vector3(transform.localPosition.x, 0f, transform.localPosition.z), new Vector3(currentTarget.transform.localPosition.y, 0f, currentTarget.transform.localPosition.z));
                    currentTarget.GetComponent<MeshRenderer>().material = targetMaterial;
                }
            }
        }
